...
func _ready() -> void:
	...
	spellbook[SignalBus.Element.find_key(SignalBus.Element.Water)] = {"poly" : new_bullet_poly, "percentage" : .05}
	spellbook[SignalBus.Element.find_key(SignalBus.Element.Love)] = {"poly" : new_bullet_poly, "percentage" : .05}
	spellbook[SignalBus.Element.find_key(SignalBus.Element.Light)] = {"poly" : new_bullet_poly, "percentage" : .05}
	spellbook[SignalBus.Element.find_key(SignalBus.Element.Song)] = {"poly" : new_bullet_poly, "percentage" : .05}
...
func trigger_cooldown(ele : SignalBus.Element):
	match ele:
		SignalBus.Element.Water:
...
func _unhandled_input(event : InputEvent) -> void:
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		trigger_cooldown(element_screenshot)
